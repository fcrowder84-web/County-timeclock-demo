...
function createQuickPunchRouter({ requireUser, requireAnyPermission, pool, audit }) {
  ...
  router.post('/clock-in', requireUser, canPunch, async (req, res) => {
    try {
      ...
      return res.json({
        message: `${req.user.first_name} clocked in successfully`,
        entry: result.rows[0],
        metadata_recorded: Boolean(metadata),
      });
    } catch (err) {
      // Migration 009 adds a partial unique index as the final guard against
      // two simultaneous clock-in requests creating duplicate open punches.
      if (err.code === '23505') {
        return res.status(400).json({ error: 'You are already clocked in' });
      }
      console.error(err);
  ...
  router.post('/clock-out', requireUser, canPunch, async (req, res) => {
    try {
      ...
      return res.json({
        message: `${req.user.first_name} clocked out successfully`,
        entry: result.rows[0],
        metadata_recorded: Boolean(metadata),
      });
    } catch (err) {
