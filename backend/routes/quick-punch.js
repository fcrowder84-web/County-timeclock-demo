...
function createQuickPunchRouter({requireUser,requireAnyPermission,pool,audit}){
  ...
  async function captureMetadata(req,timeEntryId,punchType){
    try{
      return await recordPunchMetadata({pool,req,employeeId:req.user.id,timeEntryId,punchType});
    }catch(err){
  ...
  router.get('/quick-status',requireUser,canPunch,async(req,res)=>{
    try{
      const [openResult,lastResult]=await Promise.all([
        pool.query(`SELECT id, clock_in FROM time_entries WHERE employee_id=$1 AND clock_out IS NULL ORDER BY clock_in DESC LIMIT 1`,[req.user.id]),
        pool.query(`SELECT clock_in, clock_out FROM time_entries WHERE employee_id=$1 ORDER BY GREATEST(clock_in,COALESCE(clock_out,clock_in)) DESC LIMIT 1`,[req.user.id]),
      ]);
  ...
  router.post('/delete-punch',requireUser,async(req,res)=>{
    ...
    try{
      const entryResult=await pool.query(`SELECT * FROM time_entries WHERE id=$1 LIMIT 1`,[entryId]);
      ...
      await audit(req.user.id,'delete_time_entry','time_entry',entry.id,{
  ...
  router.post('/clock-in',requireUser,canPunch,async(req,res)=>{
    try{
      const openEntry=await pool.query(`SELECT id FROM time_entries WHERE employee_id=$1 AND clock_out IS NULL ORDER BY clock_in DESC LIMIT 1`,[req.user.id]);
