...
function legacyPermissionsForRole(role) {
  ...
  let permissions = [...PERMISSION_GROUPS.employee];
  if (["supervisor", "payroll", "admin"].includes(normalized)) {
    permissions.push(...PERMISSION_GROUPS.supervisor);
  }
  if (["payroll", "admin"].includes(normalized)) {
    permissions.push(...PERMISSION_GROUPS.payroll);
  }
...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    const permissions = legacyPermissionsForRole(user.role);
    const token = createSession(user.id, permissions, {
      app_admin_scope: user.role === "admin" ? "all" : "own",
      auth_source: "legacy",
    });
