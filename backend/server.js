...
async function getPayPeriodConfig() {
  ...
  if (!config?.anchor_date || !config?.period_days) throw new Error("Pay period settings are incomplete");
  return config;
}
...
function selfApprovalRouteAllowed(req, keys) {
  ...
  if (
    keys.includes("approve_punch_correction") &&
    permissions.has("approve_own_punch_corrections") &&
    (path.endsWith("/supervisor/approve-change-request") || path.endsWith("/supervisor/approve-single-punch"))
  ) {
    return true;
  }

  if (
    keys.includes("approve_timecard") &&
    permissions.has("approve_own_timecard") &&
    path.endsWith("/supervisor/approve-timecard")
  ) {
    return true;
  }
...
function requireAnyPermission(...permissionKeys) {
  ...
  return (req, res, next) => {
    if (userHasAnyPermission(req.user, keys) || selfApprovalRouteAllowed(req, keys)) return next();
...
async function isSameDepartment(user, employeeId) {
  const result = await pool.query(
    `SELECT 1 FROM employees target WHERE target.id=$1 AND target.department_id=$2 LIMIT 1`,
...
async function isDepartmentHead(user, departmentId = null) {
  const result = await pool.query(
    `SELECT 1 FROM department_heads dh
      WHERE dh.employee_id=$1 AND dh.active=TRUE
...
async function isAssignedEmployee(user, employeeId) {
  const result = await pool.query(
    `SELECT 1 FROM supervisor_employee_assignments sea
      WHERE sea.employee_id=$1 AND sea.supervisor_employee_id=$2 AND sea.active=TRUE
      LIMIT 1`,
    [employeeId, user.id],
  );
  return result.rows.length > 0;
...
async function canAccessEmployee(user, employeeId, actionPermissions = []) {
  ...
  if (targetDepartment.rows.length && await isDepartmentHead(user, targetDepartment.rows[0].department_id)) return true;
  ...
  if (requested.some((key) => permissions.has(key)) && (await isSameDepartment(user, employeeId))) return true;
...
async function syncPortalUser(payload) {
  ...
  if (payload.department_id) {
    const byPortalId = await pool.query(`SELECT id FROM departments WHERE portal_department_id=$1 LIMIT 1`, [payload.department_id]);
  ...
  let existing = await pool.query(`SELECT id FROM employees WHERE portal_user_id=$1 LIMIT 1`, [payload.sub]);
  if (!existing.rows.length && payload.employee_number) {
    existing = await pool.query(
      `SELECT id FROM employees
        WHERE portal_user_id IS NULL AND employee_number=$1 LIMIT 1`,
  ...
  if (existing.rows.length) {
    const result = await pool.query(
      `UPDATE employees
          SET portal_user_id=$1,employee_number=$2,first_name=$3,last_name=$4,email=$5,
              department=$6,department_id=$7,portal_department_id=$8,role=$9,active=TRUE,
              is_active=TRUE,must_change_pin=FALSE,portal_permissions=$10::jsonb,
              app_admin_scope=$11,auth_source='portal',last_portal_sync_at=NOW()
        WHERE id=$12 RETURNING *`,
      [payload.sub,payload.employee_number?String(payload.employee_number):null,payload.first_name,payload.last_name,
       payload.email||null,departmentName,departmentId,payload.department_id||null,role,JSON.stringify(permissions),
  ...
  const result = await pool.query(
    `INSERT INTO employees(
     ...
     RETURNING *`,
    [payload.sub,payload.employee_number?String(payload.employee_number):null,payload.first_name,payload.last_name,
     payload.email||null,role,departmentName,departmentId,payload.department_id||null,JSON.stringify(permissions),appAdminScope],
  );
  await ensureOwnDepartmentAssignment(result.rows[0].id, departmentId, permissions);
  return { user: result.rows[0], permissions, appAdminScope };
}
...
async function syncPortalDirectory(trigger = "scheduled") {
  ...
  try {
    ...
    const current = await client.query(`SELECT COUNT(*)::int AS count FROM employees WHERE auth_source='portal' AND active=TRUE`);
    ...
    if (currentActive >= 5 && employees.length < Math.ceil(currentActive * 0.5)) {
      throw new Error(`Portal directory shrank from ${currentActive} active users to ${employees.length}; refusing automatic deactivation below 50% safety threshold`);
    }
    ...
    const removed = await client.query(
      `UPDATE employees
          SET active=FALSE,is_active=FALSE,portal_permissions='[]'::jsonb,
...
app.use(createQuickPunchRouter({ requireUser, requireAnyPermission, pool, audit }));
app.use(createLeaveRouter({ requireUser, requireAnyPermission, pool, audit, canAccessEmployee, getRequestedPayPeriod, userHasAnyPermission }));
app.use(createSupervisorRouter({ requireUser, requireAnyPermission, pool, audit, canAccessEmployee, getRequestedPayPeriod, userHasPermission }));
app.use(createPayrollRouter({ requireUser, requireAnyPermission, pool, getRequestedPayPeriod }));
app.use(createTeamStructureRouter({ requireUser, requireAnyPermission, pool, audit, canManageTeamStructure, userPermissionSet }));
...
app.get("/pay-periods", requireUser, async (req, res) => {
  try {
    ...
    res.json({ current, periods: periods.reverse() });
  } catch (err) {
    console.error(err);
...
app.use(createEmployeeRouter({ requireUser, requireAnyPermission, pool, audit, getRequestedPayPeriod }));
...
app.post("/employee/change-pin", requireUser, async (req, res) => {
  ...
  try {
    await pool.query(`UPDATE employees SET pin=$1,must_change_pin=false WHERE id=$2`, [new_pin, req.user.id]);
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      await pool.query(`UPDATE employees SET pin='1111',must_change_pin=true WHERE id=$1`, [employee_id]);
      res.json({ message: "PIN reset to 1111. Employee must change PIN at next login." });
    } catch (err) {
      console.error(err);
