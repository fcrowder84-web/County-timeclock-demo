...
async function canManageTeamStructure(user,departmentId=null){
  ...
  if(userHasPermission(user,'app_admin')||role==='timeclock_manager'||role==='payroll') return true;
...
async function canAccessEmployee(user,employeeId,actionPermissions=[]){
  ...
  // App Admin is countywide for every application capability.
...
app.use(createAuthRouter({requireUser,sessionStore,getBearerToken,verifyPortalToken,portalIssuer:PORTAL_ISSUER,portalAudience:PORTAL_AUDIENCE,syncPortalUser,getUserById,audit}));app.use(createQuickPunchRouter({requireUser,requireAnyPermission,pool,audit}));app.use(createLeaveRouter({requireUser,requireAnyPermission,pool,audit,canAccessEmployee,getRequestedPayPeriod,userHasAnyPermission}));app.use(createLunchRouter({requireUser,requireAnyPermission,pool,audit,canAccessEmployee}));app.use(createSupervisorRouter({requireUser,requireAnyPermission,pool,audit,canAccessEmployee,getRequestedPayPeriod,userHasPermission}));app.use(createPayrollRouter({requireUser,requireAnyPermission,pool,audit,canAccessEmployee,getRequestedPayPeriod}));app.use(createTeamStructureRouter({requireUser,requireAnyPermission,pool,audit,canManageTeamStructure,userPermissionSet}));
