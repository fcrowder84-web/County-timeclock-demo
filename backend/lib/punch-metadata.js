...
function getRequestIp(req){return normalizeIp(req.headers?.['cf-connecting-ip'])||normalizeIp(req.headers?.['x-forwarded-for'])||normalizeIp(req.socket?.remoteAddress)||null;}
function normalizeLocation(body={}){const rawStatus=String(body.location_status||'unavailable').toLowerCase();const status=LOCATION_STATUSES.has(rawStatus)?rawStatus:'error';const latitude=body.latitude==null?null:Number(body.latitude);const longitude=body.longitude==null?null:Number(body.longitude);const accuracy=body.accuracy_meters==null?null:Number(body.accuracy_meters);const validCoordinates=Number.isFinite(latitude)&&latitude>=-90&&latitude<=90&&Number.isFinite(longitude)&&longitude>=-180&&longitude<=180;const validAccuracy=accuracy==null||(Number.isFinite(accuracy)&&accuracy>=0);if(status==='captured'&&!validCoordinates)return {location_status:'error',latitude:null,longitude:null,accuracy_meters:null};return {location_status:status,latitude:validCoordinates?latitude:null,longitude:validCoordinates?longitude:null,accuracy_meters:validCoordinates&&validAccuracy?accuracy:null};}
function normalizeClientSource(value){const source=String(value||'web').trim().toLowerCase();return /^[a-z0-9_-]{1,40}$/.test(source)?source:'web';}
function defaultTrustedNetworks(){return (trustedNetworkConfig.trusted_networks||[]).filter(item=>item&&item.enabled!==false&&normalizeIp(item.ip)).map(item=>({name:String(item.name||item.ip),ip:normalizeIp(item.ip)}));}
async function trustedNetworks(pool){if(!pool)return defaultTrustedNetworks();try{const result=await pool.query('SELECT value FROM settings WHERE key=$1 LIMIT 1',['trusted_networks']);if(!result.rows.length||!result.rows[0].value)return defaultTrustedNetworks();const parsed=JSON.parse(result.rows[0].value);if(!Array.isArray(parsed))return defaultTrustedNetworks();return parsed.filter(item=>item&&normalizeIp(item.ip)).map(item=>({name:String(item.name||item.ip),ip:normalizeIp(item.ip)}));}catch(err){console.error('Trusted network settings lookup failed; using packaged defaults',err.message);return defaultTrustedNetworks();}}
async function getSettingJson(pool,key,fallback){if(!pool)return fallback;try{const result=await pool.query('SELECT value FROM settings WHERE key=$1 LIMIT 1',[key]);if(!result.rows.length||!result.rows[0].value)return fallback;return JSON.parse(result.rows[0].value);}catch(err){console.error('Location setting lookup failed; using default for '+key,err.message);return fallback;}}
