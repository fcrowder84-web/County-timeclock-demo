...
assert.strictEqual(punchLocationGate({headers:{'cf-connecting-ip':'64.139.245.9'},body:{location_status:'denied'}}).allowed,true);
assert.strictEqual(punchLocationGate({headers:{'cf-connecting-ip':'192.0.2.25'},body:{location_status:'denied'}}).allowed,false);
assert.strictEqual(punchLocationGate({headers:{'cf-connecting-ip':'192.0.2.25'},body:{location_status:'captured',latitude:33.77,longitude:-81.93,accuracy_meters:12}}).allowed,true);
