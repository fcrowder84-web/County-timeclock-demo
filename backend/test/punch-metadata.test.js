...
const pool={query:async(sql,args)=>{const key=args?.[0];if(key==='force_gps')return {rows:[{value:'true'}]};if(key==='trusted_networks')return {rows:[{value:JSON.stringify([{name:'Test network',ip:'203.0.113.77'}])}]};return {rows:[]};}};
...
const gpsOffPool={query:async(sql,args)=>{const key=args?.[0];if(key==='force_gps')return {rows:[{value:'false'}]};if(key==='trusted_networks')return {rows:[{value:'[]'}]};return {rows:[]};}};
assert.strictEqual((await punchLocationGate({headers:{'cf-connecting-ip':'192.0.2.25'},body:{location_status:'denied'}},gpsOffPool)).allowed,true);
const geoPool={query:async(sql,args)=>{const key=args?.[0];if(key==='force_gps')return {rows:[{value:'true'}]};if(key==='geofence_enforcement')return {rows:[{value:'true'}]};if(key==='geofences')return {rows:[{value:JSON.stringify([{id:'eoc',name:'EOC',latitude:33.77,longitude:-81.93,radius_feet:1000,enabled:true}])}]};return {rows:[]};}};
