function renderTimecard(){
  ...
  days.forEach((d,i)=>{
    ...
    const lunchFinalized=Boolean(data.approval?.payroll_finalized_at);\n    const canWaiveLunch=!ownTimecard&&currentMode==="supervisor"&&hasAny(["edit_employee_time","edit_payroll_time"])&&(!lunchFinalized||has("reopen_timecard"))&&Number(lunch?.forced_lunch_deducted_hours||0)>0&&!lunchPending;
