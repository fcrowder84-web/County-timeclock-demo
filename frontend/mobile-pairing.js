(function(){
  ...
  async function renewTrustedSession(){
    ...
    refreshPromise=(async()=>{
      try{
        const response=await nativeFetch(`${apiBase}/auth/device-session`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({device_credential:credential})});
        const data=await response.json().catch(()=>({}));
        if(!response.ok||!data.token){localStorage.removeItem(tokenKey);if(response.status===401||response.status===403)localStorage.removeItem(deviceKey);return false}
        localStorage.setItem(tokenKey,data.token);return true;
      }catch(_){return false}finally{refreshPromise=null}
    })();
    return refreshPromise;
  }

  window.TimeClockTrustedReady=(async()=>{if(!getToken()&&getDeviceCredential())await renewTrustedSession()})();
  ...
  function createDesktopModal(){
    ...
    overlay.innerHTML=`<div style="background:#fff;border-radius:14px;max-width:460px;width:100%;padding:28px;text-align:center;box-shadow:0 18px 50px rgba(0,0,0,.25)"><h2 style="margin-top:0">Phone Login Code</h2><p style="margin-bottom:8px">On the phone, open <strong>Mobile Quick Punch</strong> and choose <strong>Enter Desktop Code</strong>.</p><div id="mobilePairingCode" style="font-size:42px;font-weight:800;letter-spacing:.14em;margin:22px 0 8px;font-variant-numeric:tabular-nums">------</div><div id="mobilePairingExpiry" style="color:#667085;margin-bottom:18px">Generating code…</div><p style="font-size:14px;color:#667085">The code works once and expires after 5 minutes. After pairing, that phone stays authorized until it is logged out.</p><button type="button" id="mobilePairingClose" style="border:0;border-radius:9px;padding:10px 16px;font-weight:700;cursor:pointer">Close</button></div>`;
  ...
  async function generateDesktopCode(){
    const overlay=createDesktopModal();const codeBox=overlay.querySelector('#mobilePairingCode');const expiryBox=overlay.querySelector('#mobilePairingExpiry');overlay.style.display='flex';codeBox.textContent='------';expiryBox.textContent='Generating code…';
    try{const response=await fetch(`${apiBase}/auth/mobile-code`,{method:'POST',headers:authHeaders(),body:'{}'});const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||'Unable to generate phone login code');const code=String(data.code||'').padStart(6,'0');codeBox.textContent=`${code.slice(0,3)} ${code.slice(3)}`;const expires=new Date(data.expires_at);expiryBox.textContent=`Expires at ${expires.toLocaleTimeString([],{hour:'numeric',minute:'2-digit'})}`}catch(err){codeBox.textContent='ERROR';expiryBox.textContent=err.message||'Unable to generate code'}
  }
  ...
  function installDesktopGenerator(){
    ...
    const button=document.createElement('button');button.type='button';button.id='mobilePairingGenerateBtn';button.className=actions.classList.contains('top-actions')?'btn':'secondary';button.textContent='Phone Login Code';button.title='Generate a one-time code to authorize your phone for Mobile Quick Punch';button.addEventListener('click',generateDesktopCode);
    const printButton=[...actions.querySelectorAll('button')].find(item=>item.textContent.trim()==='Print');actions.insertBefore(button,printButton||actions.firstChild);
  }

  async function installPendingRequestAlert(){
    const actions=document.querySelector('.top-actions')||document.querySelector('#signedIn .actions');
    ...
    try{
      ...
      button.type='button';button.id='pendingRequestAlertBtn';button.className=actions.classList.contains('top-actions')?'btn btn-warn':'button';
  ...
  function installMobileRedeemer(){
    ...
    const input=box.querySelector('#mobilePairingCodeInput'),button=box.querySelector('#mobilePairingRedeemBtn'),message=box.querySelector('#mobilePairingMessage');input.addEventListener('input',()=>{const digits=input.value.replace(/\D/g,'').slice(0,6);input.value=digits.length>3?`${digits.slice(0,3)} ${digits.slice(3)}`:digits});
    async function redeem(){const code=input.value.replace(/\D/g,'');if(code.length!==6){message.textContent='Enter all 6 digits.';return}button.disabled=true;button.textContent='Connecting…';message.textContent='Checking code…';try{const response=await nativeFetch(`${apiBase}/auth/mobile-code/redeem`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({code})});const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||'Unable to use that code');localStorage.setItem(tokenKey,data.token);if(data.device_credential)localStorage.setItem(deviceKey,data.device_credential);message.textContent='Phone authorized. Loading TimeClock…';window.location.reload()}catch(err){message.textContent=err.message||'Unable to use that code';button.disabled=false;button.textContent='Use Desktop Code'}}
    button.addEventListener('click',redeem);input.addEventListener('keydown',event=>{if(event.key==='Enter')redeem()});
  }
